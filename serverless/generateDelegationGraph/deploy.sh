...
FUNCTION_NAME="generateDelegationGraph"
ZIP_FILE="function.zip"
ROLE_ARN="arn:aws:iam::152769399840:role/service-role/generateDelegationGraph-role-sxwkz5h2"

# Check if required files exist
if [ ! -f "generateDelegationGraph.js" ]; then
    echo "Error: generateDelegationGraph.js not found in current directory ($(pwd))"
    exit 1
fi

# Create deployment package
rm -f $ZIP_FILE  # Remove any existing zip file
zip -r $ZIP_FILE generateDelegationGraph.js
...
if ! aws lambda get-function --function-name $FUNCTION_NAME --region $REGION --no-cli-pager >/dev/null 2>&1; then
    ...
    aws lambda create-function \
        ...
        --runtime nodejs22.x \
        --role $ROLE_ARN \
        --handler generateDelegationGraph.handler \
