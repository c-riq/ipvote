...
FUNCTION_NAME="aggregateDelegationEdges"
ZIP_FILE="function.zip"
ROLE_ARN="arn:aws:iam::152769399840:role/service-role/aggregateDelegationEdges-role-it68a5pl"

# Check if required files exist
if [ ! -f "aggregateDelegationEdges.js" ]; then
    echo "Error: aggregateDelegationEdges.js not found in current directory ($(pwd))"
    exit 1
fi

# Create deployment package
rm -f $ZIP_FILE  # Remove any existing zip file
zip -r $ZIP_FILE aggregateDelegationEdges.js
...
if ! aws lambda get-function --function-name $FUNCTION_NAME --region $REGION --no-cli-pager >/dev/null 2>&1; then
    ...
    aws lambda create-function \
        ...
        --runtime nodejs22.x \
        --role $ROLE_ARN \
        --handler aggregatePublicProfiles.handler \
