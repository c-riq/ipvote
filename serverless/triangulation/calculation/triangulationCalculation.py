...
def lambda_handler(event, context):
    """
    AWS Lambda handler that processes triangulation data and returns a plot and country list
    """
    try:
        # Get IP address from event
        target_ip = event.get('ip')
        if not target_ip:
            return {
                'statusCode': 400,
        ...
        n_minutes_ago = current_time - (20 * 60 * 60 * 1000)

        # List and filter S3 objects
        response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix='triangulation/')
        for obj in response.get('Contents', []):
            ...
            if (contents.get('lambdaStartTimestamp', 0) > n_minutes_ago and 
                contents.get('clientIp') == target_ip):
