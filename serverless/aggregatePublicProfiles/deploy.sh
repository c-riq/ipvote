...
FUNCTION_NAME="aggregatePublicProfiles"
ZIP_FILE="function.zip"
ROLE_ARN="arn:aws:iam::152769399840:role/service-role/aggregatePublicProfiles-role-daq4cvth"

# Check if required files exist
if [ ! -f "aggregatePublicProfiles.js" ]; then
    echo "Error: aggregatePublicProfiles.js not found in current directory ($(pwd))"
    exit 1
fi

# Create deployment package
rm -f $ZIP_FILE  # Remove any existing zip file
zip -r $ZIP_FILE aggregatePublicProfiles.js
